    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      adjustPaymentWrapperPosition: function(evt) {
        ...
        if(window.innerWidth < 768) {
          if(this.paymentWrapperHeight <= 0) {
            this.paymentWrapperHeight = paymentWrapper.offsetHeight;
            paymentWrapperPositionHandler.style.height = this.paymentWrapperHeight + "px";
          }

          if(paymentWrapperPositionHandler.offsetTop < window.scrollY - this.paymentWrapperHeight) {
...
const spanClose = document.getElementById("personalized-modal__close")
spanClose.onclick = function() {
	document.getElementById("personalized-modal").style.display = "none";
	document.body.classList.remove("disable-event")
	document.getElementById('HeaderCart').classList.remove("disable-event")
}


function scrollToRight(evt) {
	let product_grid = evt.target.closest(".page-width")
	product_grid.querySelector('.scrollable-grid--small').scrollLeft += 270;
}
...
function productScrollToLeft(){
	document.querySelector(".product__thumb-arrow--prev").click()
}

function productScrollToRight(){
	document.querySelector(".product__thumb-arrow--next").click()
}

function totalSaving(){
	const cart_items = document.querySelectorAll('.cart__item-sub--js')
	let sub_total = 0
	cart_items.forEach((cart_item, index) => {
		let quantity_ele = cart_item.querySelector('.js-qty__num')
		let price_ele = cart_item.querySelector('.product-compare_at_price').firstElementChild
		let final_price_ele = cart_item.querySelector('.product-final_price').firstElementChild
		if(quantity_ele && price_ele && final_price_ele){
			let price = Number(price_ele.innerHTML.replace("$", ""))
			let final_price = Number(final_price_ele.innerHTML.replace("$", ""))
			let tmp_total = (price - final_price) * quantity_ele.value
			sub_total = sub_total + tmp_total
		}
	})
	console.log(sub_total)
	let = headerTotalSaving_mini_cart = document.getElementById('headerTotalSaving_mini_cart')
	if (headerTotalSaving_mini_cart){
		headerTotalSaving_mini_cart.innerHTML = `$${sub_total.toFixed(2)}`
	}
	let = headerTotalSaving_cart = document.getElementById('headerTotalSaving_cart')
	if (headerTotalSaving_cart) {
		headerTotalSaving_cart.innerHTML = `$${sub_total.toFixed(2)}`
	}
}
